...
public class M_share_1_1create extends AppCompatActivity {


    String TAG = "M_share_1_1create";
    EditText et_roomName, et_roomPW;  //방장이 방이름, 비번
    Button btn_createRoomDone;  //방만드는 버튼, 정규식확인, 서버에 요청보내기
        ...
        btn_createRoomDone.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                ...
                if(isPattern(get방이름, 2,12) && isPattern(get방비번, 4,8)) {
                    ...
                    retrofit_방중복체크(get방이름, get방비번, myEmail); //통신 - 방이름, 방비번 서버에 전달하기
    ...
    //중복체크만(방생성은 경로마커 이후)
    public void retrofit_방중복체크(String name, String pw, String email) {
        ...
        //서버 DB에 select문 날리기 (느리면 스레드로 빼기)
        Sharing sharing_interface = retrofit.create(Sharing.class);   // 레트로핏 인터페이스 객체 구현
        Call<Sharing_room> call = sharing_interface.checkRepetition(name); //방이름만 중복체크
        ...
        call.enqueue(new Callback<Sharing_room>() { //enqueue : 비동기식 통신을 할 때 사용/ execute: 동기식
            ...
            public void onResponse(Call<Sharing_room> call, Response<Sharing_room> response) {
                ...
                if(response.isSuccessful()) {
                    ...
                    Log.e(TAG, "통신 성공 /response : " + result.getResponse());

                    //중복이면 Toast
                    if(!result.getResponse().equals("0")) { //방이름, 삽입완료되어야만 activate값이 1이 나옴
                    ...
                    //중복이 아니라면 지도로 이동만하기
                        ...
                        Intent intent = new Intent(getApplicationContext(), M_share_2_Map.class); // -->위치공유지도
